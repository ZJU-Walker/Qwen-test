#!/usr/bin/env python3
"""DAgger data collection for the EE6D-ERA (2-wrist) checkpoints: RTC rollout +
pedal-triggered kinesthetic intervention.

Differences from dagger_record_rtc.py (the joint-space/1-wrist original):
  * Uploads the CURRENT left-wrist frame (`wrist_left`) with every request -- the 2-wrist
    server rejects requests without it. A startup pre-flight verifies the robot config
    actually exposes cam_left_wrist (also required so the RECORDING contains the left
    wrist video that ee6d retraining needs).
  * RTC cadence matches the deployed ee6d client (CONTROL_HZ 12, d=8, s=8) so rollouts
    are on-policy for the model being DAgger'd; the recording is honestly 12 fps.
  * Fresh REPO_ID default so 2-wrist recordings never mix into the old dataset.
NOTHING else changes: the wire protocol and the recorded state/action stay ABSOLUTE
JOINTS (14-dim, right arm in 7:14) -- EE conversion is entirely server-side at inference
and dataset-load-side at training.
...
Run on the ROBOT computer in the `lerobot` conda env:
    conda activate lerobot && python dagger_record_rtc_ee6d.py
"""
...
SEND_STATE_HISTORY = True       # --state_history checkpoints (0804dag run onward)
...
# Deployment parity: identical cadence to client_action_expert_rtc_ee6d_use_me.py so the
# DAgger rollout distribution IS the eval distribution (12 Hz, d=8 <= trained rtc10, s=8).
CONTROL_HZ = 12                 # control cadence; one action / one recorded frame per tick
# --- RTC scheduling (same semantics as client_action_expert_rtc_ee6d_use_me.py) ---
DELAY_STEPS = 8                 # ticks of inference latency planned around (<= trained prefix len)
EXEC_HORIZON = 8                # ticks between replans (>= DELAY_STEPS)
NUM_FLOW_STEPS = 10
...
START_SOUND = True
# --- dataset ---
REPO_ID = "trossen_data/dagger_block_ee6d"
...
def post_request(session, cam_high_frames, frame_ids, wrist_rgb, wrist_left_rgb, state_history,
                 state7, action_prefix) -> ChunkResult:
    """Encode + POST one observation (worker thread). Same protocol as the ee6d RTC client:
    `action_prefix` is (d, 7) ABSOLUTE joints or None (cold start); `wrist_left_rgb` is the
    current left-wrist frame (or None for 1-wrist checkpoints); `frame_ids` enables the
    server-side frame cache (409 => re-send everything once). Parameter order matches
    request_args() so `post_request(session, *request_args(obs), prefix)` stays valid."""
    data = {
...
class EpisodeRecorder:
    ...
    def park_episode(self) -> bool:
        ...
        entry = {
            ...
            "client": "dagger_record_rtc_ee6d",
            "sends_wrist_left": SEND_LEFT_WRIST,
            "sends_state_history": SEND_STATE_HISTORY,
...
def main():
    ...
    # Pre-flight: the 2-wrist checkpoint needs the REAL left-wrist image at inference AND
    # in the recording (ee6d retraining reads cam_left_wrist video). Fail fast and loud.
    if SEND_LEFT_WRIST and f"observation.images.{WRIST_LEFT_CAM}" not in obs0:
        ...
        raise RuntimeError(
            ...
            "Add it to TrossenAIRightArmOnlyRobotConfig in the robot's lerobot fork "
            "(mirror cam_right_wrist / the trossen_ai_solo recording config). Do NOT "
            "zero-spoof it and do NOT set SEND_LEFT_WRIST=False for a 2-wrist checkpoint.")
    ...
    state_hist: deque = deque(maxlen=NUM_FRAMES)
    ...
    def prime_history(obs) -> None:
        ...
        The state buffer primes in lockstep, mirroring training's edge clamp (repeat the
        first state). Used ONLY at episode starts: post-intervention resumes keep both
        buffers intact so the correction stays in the model's context."""
        frame, fid = cam_rgb(obs, HL_CAM), next(next_frame_id)
        history.clear(), history_ids.clear(), state_hist.clear()
        history.extend([frame] * NUM_FRAMES)
        history_ids.extend([fid] * NUM_FRAMES)
        state_hist.extend([obs["observation.state"].detach().cpu().numpy().astype(np.float32)]
                          * NUM_FRAMES)
    ...
    def request_args(obs):
        ...
        return frames, ids, cam_rgb(obs, WRIST_CAM), left, shist, state7
    ...
    settle_left = 0
    wait_ticks = 0
    hist_tick = 0               # recorded ticks this episode; drives the history cadence
    ...
    def start_episode() -> None:
        ...
        nonlocal state, plan, global_step, next_request_step, idle_grav, current_subtask, \
            last_rtt, hist_tick
        ...
        try:
            ...
            frames, ids, wrist, wleft, shist, st7 = request_args(obs0)
            result = executor.submit(post_request, session, frames, ids, wrist, wleft,
                                     shist, st7, None).result()
        except Exception as e:  # noqa: BLE001
    ...
    try:
        ...
        while not quit_requested:
            ...
            # Unified history feed (2026-08-05): the window advances every STRIDE_TICKS
            # RECORDED ticks in EVERY active phase -- policy, holds, interventions, settle,
            # resume -- exactly how training windows slide over the recorded frames. The
            # correction therefore stays in context across a resume (no reset, no amnesia).
            if IMAGE_HISTORY and hist_tick > 0 and hist_tick % STRIDE_TICKS == 0:
                history.append(cam_rgb(obs, HL_CAM))
                history_ids.append(next(next_frame_id))
                state_hist.append(state7.copy())
            hist_tick += 1

            if state == SETTLE:
                settle_left -= 1
                if settle_left <= 0:
                    # NO history reset (2026-08-05): training windows in the DAgger data
                    # slide straight through the correction frames, so the resumed policy
                    # must see the intervention in its history -- resetting to repeated
                    # current frames would be amnesia the training distribution never has.
                    # (The PLAN is still discarded: delta actions from a pre-correction
                    # state would snap the arm back. Only the observation window persists.)
                    frames, ids, wrist, wleft, shist, st7 = request_args(obs)
                    pending = PendingRequest(
                        future=executor.submit(post_request, session, frames, ids, wrist, wleft,
                                               shist, st7, None),
            ...
            if state == POLICY:
                ...
                if pending is not None and global_step >= pending.anchor + DELAY_STEPS:
                    ...
                    # (history feed moved to the unified per-recorded-tick block above)
                    if IMAGE_HISTORY and hist_tick % STRIDE_TICKS == 1:
