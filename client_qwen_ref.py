...
# --- Configuration ---
SERVER_URL = "http://10.79.12.149:8001/predict"
PROMPT = "which colored block did the human hand point to?"

CAMERA_FPS = 30
STRIDE = 15                 
INFERENCE_INTERVAL = 0.001    
BUFFER_SIZE = 5
RATE_WINDOW = 10

# --- Shared State ---
# Now stores a tuple: (raw_numpy_frame, compressed_jpeg_bytes)
...
def build_dashboard(frames, prediction):
    ...
    history_count = BUFFER_SIZE - 1  # 4 smaller history frames
    hist_w = main_w // history_count # 160
    hist_h = int(hist_w * (main_h / main_w)) # 120
    
    # Create black canvas (Main Image + History Row + Header/Footer Text)
    ...
    # 1. Top Header
    cv2.putText(canvas, "Qwen 2.5-VL Edge Inference Stream", (15, 25), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)

    # 2. Main Frame (The newest frame at index -1)
    main_img = cv2.resize(frames[-1], (main_w, main_h))
    canvas[40:40+main_h, 0:main_w] = main_img

    # 3. History Grid (The older frames)
...
def inference_consumer():
    """Background thread: sends data, updates prediction, and reports measured inference rate."""
    ...
    print(f"Network thread active: Pinging server every {INFERENCE_INTERVAL}s")

    while True:
        start_time = time.time()
        
        # Safely grab the compressed JPEGs from the buffer tuples
        with state_lock:
        ...
        try:
            ...
            if response.status_code == 200:
                # Safely update the text that the GUI is reading
                with state_lock:
...
def main_gui_loop():
    ...
    if not cap.isOpened():
        print("Camera fault. Exiting.")
        return

    frame_count = 0
    is_initialized = False
    
    print(f"GUI started: {CAMERA_FPS}Hz hardware, Stride {STRIDE}")
    ...
    try:
        while True:
            ...
            with state_lock:
                # Rule 1: Duplicate first frame
                if not is_initialized:
                    for _ in range(BUFFER_SIZE):
                        # We save BOTH the raw frame (for GUI) and compressed bytes (for network)
                        frame_buffer.append((frame, img_bytes))
                    is_initialized = True
                    
                # Rule 2: Stride Update
                elif frame_count % STRIDE == 0:
                    frame_buffer.append((frame, img_bytes))

                # Grab safe copies for drawing the dashboard
            ...
            cv2.imshow("Qwen-VL Real-Time Robotics View", dashboard)
...
if __name__ == "__main__":
    # Launch Network Consumer in the background
    net_thread = threading.Thread(target=inference_consumer, daemon=True)
    net_thread.start()

    # Run OpenCV GUI on the main thread
    main_gui_loop()
